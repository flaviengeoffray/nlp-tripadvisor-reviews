...
class NaiveBayesModel(BaseClassificationModel):
    ...
    def fit(
        ...
        if isinstance(y_train, list) and all(isinstance(label, str) for label in y_train[:5]):
        ...
        if X_val is not None and y_val is not None:
            ...
            if isinstance(y_val, list) and all(isinstance(label, str) for label in y_val[:5]):
